...
class PHPMailer
{
     ...
     * @returns void
     */
    function AddCC($addresses, $names = null ) {
    	$idx = 0;
    	foreach ($addresses as $addr) {
	        $this->cc[$idx][0] = trim($addr);
	        if ($names != null) {
		        $this->cc[$idx][1] = $names[$idx];
	        } else {
	        	$this->cc[$idx][1] = "";
	        }
	        $idx++;
    ...
    function addr_append($type, $addr) {
		...
		$idx_elem = 0;
        foreach ($addr as $ad) {        	
	    	$addr_str .= $this->addr_format($ad);
	    	if ( ($idx_elem < $ult_elem-1) ) {
        		$addr_str .= ', ';
	    	} else if ( ($idx_elem = $ult_elem) ) {
        		$addr_str .= $this->LE;
	    	} 
/*	        if(count($ad) > 1) {
	        	echo"vasio, phpmailer 782";
	        }*/
	        $idx_elem++;
        }     
        return($addr_str);        
